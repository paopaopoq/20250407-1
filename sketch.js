...
function createMenu() {
  ...
  let portfolioButton = createButton('作品集').parent(menu).style('background-color', '#FFC0CB').style('font-size', '20px').style('padding', '10px 20px').style('margin', '5px').style('border', 'none').style('border-radius', '5px');
  ...
  introButton.mousePressed(() => {
    ...
    if (showIntro) {
      introText = '大家好，我是林家妤，烏薩奇媽，獅子座，座標北部，喜歡看漫畫、電影、動漫、玩電腦遊戲。';
    } else {
      introText = ''; // 清空文字
    }
  });
